...
public class Department implements CrawlerDown {
	...
	public void statisticsDistrictsCode() {
		...
		String content = request.doGet(Linking.DEPARTMENT.getUrl());
	...
	private void writeToChinaTxt(CNode node, CNode pNode) {
		...
		String sql = "insert into china (name,code,parentcode) values ('" + node.getName() + "','" + node.getCode()
				+ "','" + pNode.getCode() + "')";
